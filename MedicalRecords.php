      ...
      <?php if ($docMeds) {
        foreach ($docMeds as $docMed) :   ?>
          <div class="Doc_teams">
            ...
            <div class="doc_about">
              <i class="fa fa-calendar"></i> <?php echo date('d F, Y h:i A', strtotime($docMed['treat_date'])); ?><br>
            </div>
            <div class="btn-doctor">
              <a href="EditMedRec.php?ap=<?php echo $docMed['app_id']; ?>&md=<?php echo $docMed['med_id']; ?>" style="text-decoration: none;"><button class="btn-1">Edit Medical Record</button></a>
            </div>
          </div>
        <?php endforeach;
      } else { ?>
